...
# WARP DIRECTORY (vendored from github.com/mfaerevaag/wd v0.10.0)
# ==============================================================
# Jump to custom directories in the terminal because `cd` takes too long.
#
# Original upstream layout was three files (wd.zsh, wd.sh, _wd):
#   - wd.zsh: tiny loader that lazy-sourced wd.sh on each `wd` call
#   - wd.sh:  full implementation, re-sourced per invocation
#   - _wd:    completion (was named `_wd.sh` here, which broke autoloading
#             and was silently dead — removed during this merge)
# Merged into one file: helper/core functions live at top level,
# the per-invocation dispatch logic is wrapped in `wd()` below.
...
wd_browse() {
    ...
    # --expect=delete makes fzf print the pressed key as the first line of
    # output. We then dispatch in this shell (where `wd` is a function),
    # since fzf's --bind=execute(...) runs in a subshell that wouldn't see it.
    local fzf_out
    fzf_out=$(printf '%s\n' "${entries[@]}" \
        | fzf --height 100% --reverse --expect=delete)
    [[ -z $fzf_out ]] && return 0

    local key="${fzf_out%%$'\n'*}"           # 1st line: pressed key (empty for Enter)
    local selected_entry="${fzf_out#*$'\n'}" # 2nd line: chosen entry
    local selected_point="${selected_entry%% ->*}"
    selected_point="${selected_point## }"
    selected_point="${selected_point%% }"

    if [[ $key == delete ]]; then
        wd rm "$selected_point"
...
# Main entry point. Re-runs argument parsing + warp-point loading on every
# call, just like the original upstream wd.sh did when re-sourced per call.
wd() {
    WD_CONFIG=${WD_CONFIG:-$HOME/.warprc}
    local WD_QUIET=0
    local WD_EXIT_CODE=0
    local wd_extglob_is_set
    ...
    # restore extendedglob if we toggled it
    if (( wd_extglob_is_set == 0 )); then
        setopt extendedglob
    fi
...
# ----- completion -----
# Just commands — warp points are reachable faster via the Ctrl-B fzf widget.
# Relies on compinit having already run (via completion.zsh, sourced earlier).
_wd() {
    local -a commands
    commands=(
        'add:Add current directory as a warp point'
        'addcd:Add a path as a warp point'
        'rm:Remove the given warp point'
        'list:List all stored warp points'
        'ls:List files in given warp point'
        'open:Open warp point in default file explorer'
        'path:Print path of given warp point'
        'show:Show warp points pointing at current dir, or path of given point'
        'browse:Interactive fzf picker for warp points'
        'clean:Remove warp points whose target directory is gone'
        'export:Export warp points as static named directories'
        'help:Show usage'
    )
    _describe -t commands "wd commands" commands
}
